...
@router.get("/", response_model=PaginatedResult[schemas.UserRead], summary="获取用户列表", description="获取所有用户")
...
@router.get("/{user_id}", response_model=schemas.UserReadWithRoles, summary="根据用户ID获取用户", description="根据用户ID获取用户详情")
...
@router.post("/", response_model=schemas.UserRead, summary="创建用户", description="创建新用户")
...
@router.put("/{user_id}", response_model=schemas.UserRead, summary="更新用户", description="更新用户信息")
...
@router.delete("/{user_id}", summary="删除用户", description="删除用户", status_code=204)
async def delete_user(user_id: int, db: AsyncSession = db_depends):
    """删除用户"""
    await controllers.delete_user(db, user_id)


@router.post("/{user_id}/roles", response_model=schemas.UserReadWithRoles, summary="为用户分配角色", description="为用户分配角色")
...
@router.get("/{user_id}/permissions", response_model=list[PermissionRead], summary="获取用户权限", description="获取用户权限")
