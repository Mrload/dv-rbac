...
@router.post("/", response_model=schemas.RoleRead, summary="创建新角色", description="创建一个新的角色, 并返回创建后的角色信息")
...
@router.get("/", response_model=PaginatedResult[schemas.RoleRead], summary="获取角色列表", description="获取所有角色的分页列表")
...
@router.get("/{role_id}", response_model=schemas.RoleReadWithPermissions, summary="根据ID获取角色", description="根据角色ID获取角色信息")
...
@router.delete("/{role_id}", summary="删除角色", description="根据角色ID删除角色", status_code=204)
...
@router.post("/{role_id}/permissions", response_model=schemas.RoleReadWithPermissions, summary="给角色分配权限", description="给指定角色分配权限")
